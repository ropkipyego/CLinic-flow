...
export function PharmacyPage() {
  ...
  async function dispense(rx: any) {
    try {
      ...
      setSelected(null);
      setOk("Medicine dispensed. Stock and the pharmacy charge were updated.");
      await load();
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Pharmacy"
        subtitle="Dispense from the queue, or adjust stock. Every change writes a movement with the previous and new quantity."
        actions={
      ...
      {tab === "queue" ? (
        ...
        <>
          ...
          <Table headers={["Medicine", "SKU", "On hand", "Reorder", "Price"]}>
            {meds.map((m) => (
              <tr key={m.id} className={m.lowStock ? "bg-amber-50" : ""}>
